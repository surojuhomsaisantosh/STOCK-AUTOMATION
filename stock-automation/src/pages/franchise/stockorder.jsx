...
import {
  ...
  FiMinus, FiPlus, FiTrash2, FiDownload, FiRefreshCw 
} from "react-icons/fi";
...
const FullPageInvoice = ({ data, profile, orderId, companyDetails, pageIndex, totalPages, itemsChunk }) => {
  ...
  const orderTime = data.orderTime || new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true });

  const taxableAmount = data.totalTaxable || 0;
  const cgst = data.totalCGST || 0;
  const sgst = data.totalSGST || 0;
      ...
      <div className="w-full border-2 border-black flex flex-col relative h-full">
        ...
        <div className="flex border-b-2 border-black bg-slate-50 print:bg-transparent">
          <div className="w-1/2 border-r-2 border-black p-2">
            ...
            <p className="font-black text-sm">#{orderId?.toString().slice(-6).toUpperCase() || 'PENDING'}</p>
          </div>
          <div className="w-1/2 p-2">
            <span className="font-bold text-slate-500 uppercase text-[9px]">Invoice Date & Time:</span>
            <div className="flex items-baseline gap-2">
                <p className="font-black text-sm">{invDate}</p>
                <p className="font-bold text-xs text-slate-500">{orderTime}</p>
            </div>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <thead className="bg-slate-100 text-[10px] border-b-2 border-black">
              <tr>
                ...
                <th className="p-2 border-r-2 border-black w-20 text-right">Taxable Rate</th>
                <th className="p-2 w-24 text-right">Amount (Inc. Tax)</th>
              </tr>
            </thead>
            <tbody className="text-[10px] font-bold">
              {itemsChunk.map((item, idx) => (
                <tr key={idx} className="h-[35px]">
                  ...
                  <td className="p-2 border-r-2 border-b border-black uppercase truncate max-w-[150px]">
                      {item.item_name}
                      <span className="block text-[8px] text-gray-500 font-normal">GST: {item.gst_rate || 0}%</span>
                  </td>
                  <td className="p-2 border-r-2 border-b border-black text-center">{item.displayQty} {item.cartUnit}</td>
                  <td className="p-2 border-r-2 border-b border-black text-right">{formatCurrency(item.preciseTaxableRate)}</td>
        ...
        <div className="flex border-t-2 border-black mt-auto">
          <div className="w-[60%] border-r-2 border-black flex flex-col">
            <div className="p-2 border-b-2 border-black min-h-[40px] flex flex-col justify-center bg-slate-50">
              ...
              <p className="font-black italic capitalize text-[11px] mt-0.5">{amountToWords(grandTotal)}</p>
            </div>
            <div className="p-3">
          ...
          <div className="w-[40%] flex flex-col text-[10px]">
            ...
            <div className="flex justify-between p-1.5 border-b border-black"><span>CGST</span><span>{formatCurrency(cgst)}</span></div>
            <div className="flex justify-between p-1.5 border-b border-black"><span>SGST</span><span>{formatCurrency(sgst)}</span></div>
            <div className="flex justify-between p-1.5 border-b border-black"><span>Round Off</span><span>{roundOff > 0 ? '+' : (roundOff < 0 ? '-' : '')}{formatCurrency(Math.abs(roundOff))}</span></div>
            <div className="flex justify-between p-2 border-b-2 border-black bg-slate-200"><span className="font-black uppercase">Total</span><span className="font-black">{formatCurrency(grandTotal)}</span></div>
...
function StockOrder() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const cachedStocks = sessionStorage.getItem("cachedStocks");
      if (cachedStocks) {
        try {
          setStocks(JSON.parse(cachedStocks));
          setLoadingStocks(false); 
        } catch (parseError) {
          sessionStorage.removeItem("cachedStocks");
        }
      }
      ...
      if (stockData) {
        setStocks(stockData);
        sessionStorage.setItem("cachedStocks", JSON.stringify(stockData));
      }
  ...
  const calculations = useMemo(() => {
    ...
    const details = cart.map(item => {
      ...
      const isInclusive = item.sales_tax_inc?.toLowerCase() === 'inclusive'; 
      ...
      if (isInclusive) {
        ...
        taxAmount = itemTotal - taxableValue;
        taxableRate = price / (1 + (gstRate / 100));
      } else {
        taxableRate = price;
      ...
      grandTotalRaw += itemTotal;

      return { 
          ...item, 
          preciseSubtotal: itemTotal,
          preciseTaxable: taxableValue,
          preciseTaxableRate: taxableRate,
          preciseGst: taxAmount
      };
    });

    const roundedBill = Math.round(grandTotalRaw);
    const roundOff = roundedBill - grandTotalRaw;
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      for (const item of cart) {
        ...
        if (!live || live.quantity < (item.qty * getConversionFactor(item.cartUnit))) {
          throw new Error(`${item.item_name} stock changed.`);
        }
      }

      // Map cart items to match the SQL parameters (including GST)
  ...
  return (
    <>
      ...
      <div className="min-h-[100dvh] bg-[#F3F4F6] pb-24 font-sans text-black relative print:hidden">
        {orderSuccess && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            <div className="bg-white rounded-[2.5rem] p-8 max-w-sm w-full text-center shadow-2xl animate-in zoom-in-95">
              ...
              <p className="text-slate-500 font-bold text-xs mb-8">Your order #{lastOrderId?.toString().slice(0,8)} has been placed.</p>
            ...
            <div className="fixed inset-y-0 right-0 z-[70] w-full max-w-md bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
                  ...
                  <div key={item.id} className="flex gap-4 p-4 border border-slate-200 rounded-2xl bg-white shadow-sm items-center">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <h4 className="text-[12px] font-black uppercase leading-tight">{item.item_name}</h4>
                      </div>
                      <div className="flex items-center gap-4 mt-2">
                        ...
                        <p className="text-sm font-black">{formatCurrency(item.preciseSubtotal)}</p>
                      </div>
                    </div>
              ...
              {cart.length > 0 && (
                <div className="p-6 border-t border-slate-100 bg-white shadow-inner">
                  <div className="space-y-2 mb-6 p-4 bg-slate-50 rounded-2xl text-[11px] font-black uppercase border border-slate-200">
                    <div className="flex justify-between text-slate-500">
                      <span>Taxable Total</span>
                      <span>{formatCurrency(calculations.totalTaxable)}</span>
                    </div>
                    <div className="py-2 border-y border-slate-200 my-2">
                      <div className="flex justify-between text-slate-500 font-bold">
                        <span>GST Total</span>
                        <span>{formatCurrency(calculations.totalGst)}</span>
                      </div>
                    </div>
                    {Math.abs(calculations.roundOff) > 0.001 && (
                      <div className="flex justify-between text-slate-400 text-[10px]">
                        <span>Round Off</span>
                        <span>{calculations.roundOff > 0 ? '+' : (calculations.roundOff < 0 ? '-' : '')}{formatCurrency(Math.abs(calculations.roundOff))}</span>
                      </div>
                    )}
                    <div className="flex justify-between text-lg pt-2 mt-2 text-black">
                      <span>Total Amount</span>
                      <span>{formatCurrency(calculations.roundedBill)}</span>
                    </div>
                  </div>
          ...
          <nav className="max-w-7xl mx-auto px-4 py-3 flex flex-col gap-3">
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex-1 flex justify-center sm:justify-center">
                <h1 className="text-[11px] font-black uppercase tracking-[0.15em] text-black">Inventory</h1>
              </div>
            ...
            <div className="flex flex-col md:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input placeholder="SEARCH ITEMS..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full h-11 pl-12 pr-4 bg-slate-50 border border-slate-200 rounded-2xl text-[10px] font-black uppercase outline-none focus:border-black focus:bg-white transition-all shadow-sm" />
              </div>
              <div className="flex gap-2">
                <button onClick={() => setShowOnlyAvailable(!showOnlyAvailable)} className={`flex-1 sm:flex-none px-4 h-11 rounded-2xl border font-black text-[9px] uppercase flex items-center justify-center gap-2 transition-all shadow-sm ${showOnlyAvailable ? "bg-emerald-600 border-emerald-600 text-white" : "bg-white border-slate-200 text-slate-600 hover:border-slate-400"}`}>
                  <FiFilter /> {showOnlyAvailable ? "Available" : "All"}
                </button>
                <div className="hidden lg:flex items-center gap-2 bg-slate-50 px-5 h-11 rounded-2xl border border-slate-200 text-[10px] font-black uppercase shadow-sm"><FiCalendar size={16} /> {today}</div>
              </div>
            </div>

            <div className="flex gap-2 overflow-x-auto py-1 sticky top-0 bg-white/95 pb-2">
              {sortedCategories.map((cat) => (
                <button
                  key={cat}
                  onClick={() => setSelectedCategory(cat)}
                  className={`flex-shrink-0 px-5 py-2.5 rounded-xl text-[9px] font-black uppercase border-2 transition-all active:scale-95 ${selectedCategory === cat ? "text-white border-transparent shadow-lg" : "bg-white text-black border-slate-200 hover:border-black"}`}
        ...
        <main className="max-w-7xl mx-auto px-3 sm:px-4 mt-4">
          {loadingStocks ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-6 pb-20">
              {filteredStocks.map((item) => {
                ...
                return (
                  <div key={item.id} className={`group bg-white rounded-2xl sm:rounded-3xl border-2 p-3 sm:p-5 transition-all duration-300 flex flex-col relative min-h-[240px] hover:shadow-2xl hover:-translate-y-1 ${isInCart ? 'border-emerald-500 ring-4 ring-emerald-500/10' : 'border-slate-100'} ${isOutOfStock ? 'bg-slate-50/50' : ''}`}>
                    {isInCart && (
                    ...
                    <h3 className="font-black text-[11px] sm:text-[13px] uppercase leading-tight mb-1 group-hover:text-emerald-900 transition-colors line-clamp-2 h-8">{item.item_name}</h3>
                      ...
                      {!isOutOfStock ? (
                        <>
                          <div className="flex gap-1.5">
                            <div className="flex-1 flex items-center border border-slate-100 rounded-xl bg-slate-50 overflow-hidden focus-within:border-black transition-all">
                              <button onClick={() => handleQtyInputChange(item.id, null, item.quantity, true, -1)} className="px-2 h-9 hover:bg-slate-200"><FiMinus size={10} /></button>
                              <input type="number" value={qtyInput[item.id] || ""} onChange={(e) => handleQtyInputChange(item.id, e.target.value, item.quantity)} className="w-full text-center font-black text-[11px] bg-transparent outline-none p-0" placeholder="0" />
                              <button onClick={() => handleQtyInputChange(item.id, null, item.quantity, true, 1)} className="px-2 h-9 hover:bg-slate-200"><FiPlus size={10} /></button>
                            </div>
                            <select value={unit} onChange={(e) => handleUnitChange(item.id, e.target.value)} className="w-12 bg-white border border-slate-100 rounded-xl text-[9px] font-black uppercase px-0.5 outline-none cursor-pointer">
                          ...
                          <button
                            ...
                            className="w-full py-3.5 rounded-xl text-[10px] font-black uppercase tracking-[0.1em] text-white transition-all shadow-md active:scale-95"
                            style={{ backgroundColor: BRAND_COLOR }}
                          >
                            {isInCart ? "Update" : "Add"}
                          </button>
                        ...
                        <button onClick={() => !isNotified && handleNotifyMe(item)} className={`w-full py-3.5 rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-2 transition-all ${isNotified ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-red-600 text-white hover:bg-red-700'}`}>
