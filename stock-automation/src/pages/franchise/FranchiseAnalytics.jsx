...
const FullPageInvoice = ({ data, profile, orderId, companyDetails, pageIndex, totalPages, itemsChunk }) => {
  ...
  return (
    <div className={`w-full bg-white text-black font-sans p-8 print:p-6 box-border text-xs leading-normal h-full relative print:h-[297mm] ${pageIndex < totalPages - 1 ? 'print:break-after-page' : ''}`}>
      <div className="w-full border-2 border-black flex flex-col relative h-full">
        ...
        {/* Table - REVERTED TO COMBINED GST */}
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-100 text-[10px] border-b-2 border-black">
              <tr>
                ...
                <th className="p-2 border-r-2 border-black w-14 text-center">GST %</th>
                <th className="p-2 border-r-2 border-black w-20 text-right">GST Amt</th>
            ...
            <tbody className="text-[10px] font-bold">
              {itemsChunk.map((item, idx) => (
                <tr key={idx} className="h-[35px]">
                  ...
                  {/* COMBINED GST DATA CELLS */}
                  <td className="p-2 border-r-2 border-b border-black text-center">{item.gst_rate || 0}%</td>
                  <td className="p-2 border-r-2 border-b border-black text-right">{formatCurrency(item.preciseGst || 0)}</td>

                  <td className="p-2 border-b border-black text-right">{formatCurrency(item.preciseSubtotal + (item.preciseGst || 0))}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="flex border-t-2 border-black mt-auto">
          <div className="w-[60%] border-r-2 border-black flex flex-col">
            ...
            <div className="p-3 flex-1 flex flex-col justify-between">
              ...
              <div className="mt-4 pt-2 border-t border-slate-300">
                ...
                <p className="text-[9px] text-slate-600 whitespace-pre-wrap leading-tight">{companyDetails?.terms || "No terms available."}</p>
              </div>
            </div>
          </div>
          ...
          <div className="w-[40%] flex flex-col text-[10px]">
            ...
            {/* UPDATED FOOTER WITH TOTAL GST AND BREAKDOWN */}
            <div className="flex justify-between p-1.5 border-b border-slate-300"><span>Total GST</span><span>{formatCurrency(data.totalGst || 0)}</span></div>
            <div className="flex justify-between py-1 px-2 border-b border-slate-300 text-slate-600 text-[9px] bg-slate-50 pl-4"><span>CGST</span><span>{formatCurrency(cgst)}</span></div>
            <div className="flex justify-between py-1 px-2 border-b border-black text-slate-600 text-[9px] bg-slate-50 pl-4"><span>SGST</span><span>{formatCurrency(sgst)}</span></div>
...
function StockOrder() {
  ...
  // CORRECTED THE VALIDATION BYPASS BUG FOR DIRECT CART TYPING
