  ...
  return (
    <div style={styles.page}>
      <div style={styles.mainWrapper}>
        ...
        {/* HEADER */}
        <header style={{ ...styles.header, padding: isMobile ? '0 15px' : '0 24px', height: isMobile ? '70px' : '80px' }}>
          <div style={{ ...styles.headerLeft, width: isMobile ? 'auto' : '200px' }}>
          ...
          <div style={styles.headerCenter}>
            <h1 style={{ ...styles.headerTitle, fontSize: isMobile ? '16px' : '18px' }}>Support Portal</h1>
            {isMobile && <div style={{ fontSize: '9px', fontWeight: '800', color: PRIMARY, marginTop: '2px' }}>{franchiseId}</div>}
          </div>

          <div style={{ ...styles.headerRight, width: isMobile ? 'auto' : 'fit-content' }}>
            {!isMobile && (
              <>
            ...
            <button onClick={() => setShowHistory(true)} style={{ ...styles.historyBtn, padding: isMobile ? '8px' : '10px' }} title="View History">
              <History size={isMobile ? 18 : 20} />
            </button>
          </div>
        </header>

        <main ref={scrollRef} style={{ ...styles.chatArea, padding: isMobile ? '15px 12px 30px 12px' : '20px 16px 40px 16px' }}>
          <div style={{ ...styles.messageContainer, maxWidth: isMobile ? '100%' : '750px' }}>
            {chatHistory.map((chat, index) => (
              <div key={index} style={{ display: 'flex', justifyContent: chat.role === "user" ? "flex-end" : "flex-start", alignItems: 'flex-start', gap: isMobile ? '8px' : '12px', marginBottom: '4px' }}>
                {chat.role === "bot" && <div style={{ ...styles.botAvatar, width: isMobile ? '32px' : '40px', height: isMobile ? '32px' : '40px' }}><Headphones size={isMobile ? 15 : 18} /></div>}
                <div style={{ maxWidth: isMobile ? '85%' : '75%' }}>
                  <div style={{
                    ...styles.bubble,
                    backgroundColor: chat.role === "user" ? PRIMARY : "#f3f5f4",
                    color: chat.role === "user" ? WHITE : "#1a1a1a",
                    borderRadius: chat.role === "user" ? '18px 18px 2px 18px' : '2px 18px 18px 18px',
                    border: chat.isSuccess ? `1.5px solid ${PRIMARY}` : 'none',
                    padding: isMobile ? '12px 16px' : '16px 20px'
                  }}>
                    {chat.isSuccess && <CheckCircle2 size={16} style={{ marginBottom: '8px', color: PRIMARY }} />}
                    <p style={{ margin: 0, fontSize: isMobile ? '14px' : '15px', lineHeight: '1.5' }}>{chat.content}</p>
                    {chat.isMenu && (
                      <div style={{ ...styles.menuGrid, gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr' }}>
                        {menuOptions.map((opt) => (
                          <button key={opt.value} onClick={() => handleOptionClick(opt)} style={{ ...styles.menuBtn, padding: isMobile ? '12px' : '14px' }}>
                ...
                {chat.role === "user" && <div style={{ ...styles.userAvatar, width: isMobile ? '32px' : '40px', height: isMobile ? '32px' : '40px' }}><User size={isMobile ? 15 : 18} /></div>}
              </div>
            ))}
            {loading && <div style={{ ...styles.loadingState, marginLeft: isMobile ? '40px' : '52px' }}><Loader2 size={16} className="animate-spin" /><span>Processing Request...</span></div>}
          </div>
        </main>

        <footer style={{ ...styles.footer, padding: isMobile ? '15px' : '24px' }}>
          <form onSubmit={handleSendMessage} style={{ ...styles.form, maxWidth: isMobile ? '100%' : '750px' }}>
            <input
              type="text"
              value={message}
              onChange={(e) => setMessage(e.target.value)}
              placeholder={activeCategory ? `Describe your issue...` : "Select category..."}
              style={{ ...styles.input, padding: isMobile ? '14px 20px' : '18px 24px' }}
              disabled={loading || !activeCategory}
            />
            <button type="submit" disabled={loading || !message.trim() || !activeCategory} style={{ ...styles.sendBtn, width: isMobile ? '48px' : '56px', height: isMobile ? '48px' : '56px', backgroundColor: (loading || !message.trim() || !activeCategory) ? '#f0f0f0' : PRIMARY }}>
              <SendHorizontal size={isMobile ? 18 : 22} color={WHITE} />
            </button>
          </form>
        </footer>
      </div>
      ...
      {showHistory && (
        <div style={styles.modalOverlay} onClick={() => setShowHistory(false)}>
          <div style={{ ...styles.historyDrawer, width: isMobile ? '100%' : 'min(480px, 95vw)' }} onClick={(e) => e.stopPropagation()}>
            <div style={{ ...styles.drawerHeader, padding: isMobile ? '20px' : '28px' }}>
              <h2 style={{ margin: 0, fontSize: isMobile ? '16px' : '18px', fontWeight: '800' }}>Support History</h2>
              <button onClick={() => setShowHistory(false)} style={styles.closeBtn}><X size={24} /></button>
            </div>
            <div style={{ ...styles.drawerBody, padding: isMobile ? '15px' : '24px' }}>
              {pastTickets.length === 0 ? (
                ...
                <div key={t.id} style={{ ...styles.ticketCard, borderLeft: `4px solid ${t.status === 'Closed' ? PRIMARY : '#f59e0b'}` }}>
                  ...
                  <div style={{ marginBottom: '12px' }}>
                    ...
                    <p style={{ ...styles.ticketMsg, fontSize: isMobile ? '14px' : '15px' }}>{t.message}</p>
                  </div>
                  {t.reply_message && (
                    <div style={styles.replyBox}>
                      <div style={styles.replyHeader}><Headphones size={12} /><span>RESPONSE:</span></div>
                      <p style={{ ...styles.replyText, fontSize: isMobile ? '13px' : '14px' }}>{t.reply_message}</p>
                    </div>
                  )}
                  <div style={styles.ticketDate}>{new Date(t.created_at).toLocaleDateString()}</div>
                </div>
              ))}
            </div>
          </div>
...
const styles = {
  ...
  header: { display: 'flex', alignItems: 'center', borderBottom: '1px solid #f1f5f9', backgroundColor: WHITE, zIndex: 10 },
  headerLeft: {},
  headerRight: { display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '12px' },
  ...
  historyBtn: { background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '12px', cursor: 'pointer', color: PRIMARY },
  headerTitle: { fontWeight: '900', color: '#1e293b', margin: 0, textTransform: 'uppercase', letterSpacing: '0.5px' },
  ...
  chatArea: { flex: 1, overflowY: 'auto', backgroundColor: '#fff' },
  messageContainer: { margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '24px' },
  botAvatar: { borderRadius: '14px', backgroundColor: '#f1f5f9', color: PRIMARY, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  userAvatar: { borderRadius: '14px', border: `2px solid ${PRIMARY}`, color: PRIMARY, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  bubble: { fontWeight: '500', boxShadow: '0 4px 15px rgba(0,0,0,0.03)' },
  ...
  menuBtn: { backgroundColor: WHITE, borderRadius: '14px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer', fontSize: '14px', fontWeight: '700', color: '#334155', transition: '0.2s' },
  ...
  footer: { borderTop: '1px solid #f1f5f9', backgroundColor: WHITE },
  form: { margin: '0 auto', display: 'flex', alignItems: 'center', gap: '12px' },
  input: { flex: 1, background: '#f8fafc', borderRadius: '18px', border: '1px solid #f1f5f9', outline: 'none', fontSize: '15px', fontWeight: '500', color: '#1e293b' },
  sendBtn: { border: 'none', borderRadius: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' },
  loadingState: { display: 'flex', alignItems: 'center', gap: '8px', color: '#94a3b8', fontSize: '12px', fontWeight: '700' },
  ...
  historyDrawer: { backgroundColor: WHITE, height: '100%', boxShadow: '-20px 0 50px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column' },
  drawerHeader: { borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  closeBtn: { background: '#f1f5f9', border: 'none', padding: '8px', borderRadius: '10px', cursor: 'pointer', color: '#64748b' },
  drawerBody: { flex: 1, overflowY: 'auto', backgroundColor: '#f8fafc' },
  ...
  ticketMsg: { color: '#334155', margin: '6px 0', fontWeight: '600', lineHeight: 1.5 },
  ...
  replyText: { color: '#064e3b', margin: 0, lineHeight: '1.6', fontWeight: '600' },
